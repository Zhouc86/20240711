...
//#define ADAPTER_NAME "eth0"
// #define ADAPTER_NAME "eno1"//p52
...
//  ec_dcsync0(slave,   active,           cycletime,  calc and copy time)
    ec_dcsync0(slave,   true,    5000000U,   1000U);
...
void rt_ethercat_init(std::string &filename)
{
    ...
    for(i = 1; i < 100; i++)
    {
        ...
        rc = run_ethercat(filename.c_str()); // todo?
        if(rc) break;
        osal_usleep(1000000);
    }
