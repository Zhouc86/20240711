...
struct ControlFSMData {
    ControlFSMData() : position_(19), velocity_(18){
        ...
        wbic_q_des_ = Eigen::VectorXd(12);

        inekf_est_pos_.setZero();
        inekf_est_rot_.setIdentity();
        inekf_est_lin_vel_.setZero();
        inekf_est_ang_vel_.setZero();
    ...
    // for debug info
    double fsm_stop_q_des_[12];


    // for wbic
    double current_time_{0};
    Eigen::VectorXd wbic_q_des_;
    Eigen::VectorXd wbic_dq_des_;
    double wbic_torque_[12];

    double wbic_qdes_[12];
    double wbic_dqdes_[12];

    // for inekf
    Eigen::Vector3d inekf_est_pos_;
    Eigen::Vector4d inekf_est_rot_;
    Eigen::Vector3d inekf_est_lin_vel_;
    Eigen::Vector3d inekf_est_ang_vel_;

    Eigen::Vector3d imu_acc_;
    Eigen::Vector3d imu_gyr_;

    int contact_est_[4];

    Eigen::Vector3d rf_pos_;
    Eigen::Vector3d lf_pos_;
    Eigen::Vector3d rh_pos_;
    Eigen::Vector3d lh_pos_;
