...
ControlFSM::ControlFSM(const std::shared_ptr<ControlFSMData>& data){
    ...
    states_list_.stop = std::make_shared<FSM_State_Stop>(data);
    states_list_.wbic = std::make_shared<FSM_State_WBIC>(data);
...
std::shared_ptr<FSM_State> ControlFSM::getNextState(FSM_StateName stateName)
{
    switch (stateName) {
        ...
        case FSM_StateName::STOP:
            return states_list_.stop;

        case FSM_StateName::WBIC:
            return states_list_.wbic;
