...
#ifndef BAER_ETHERCAT_FSM_STATE_WBIC_H
#define BAER_ETHERCAT_FSM_STATE_WBIC_H

#include "FSM_State.h"
#include "../WBC_Ctrl/LocomotionCtrl/LocomotionCtrl.hpp"
#include "../WBC_Ctrl/WBCDataBuffer.h"
#include "gait.h"

class FSM_State_WBIC : public FSM_State {
public:
    FSM_State_WBIC(std::shared_ptr<ControlFSMData> control_data);
...
private:
    ...
    std::shared_ptr<LocomotionCtrl> wbc_ctrl_;
    std::shared_ptr<WBCDataBuffer> wbc_data_;
    std::shared_ptr<Gait> gait_;

    // for debug
    bool print_debug_info_{false};

    int walking_max_step_{2000};
    int walking_counter_{0};
};


#endif //BAER_ETHERCAT_FSM_STATE_WBIC_H
