...
void LinearKFPositionVelocityEstimator::run() {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        Vec3<double> ph = _stateEstimatorData.leg_data[i].hip_location; // hip positions relative to CoM

        //std::cout << ph << std::endl;

        Vec3<double> p_rel = ph + this->_stateEstimatorData.leg_data[i].p;
        Vec3<double> dp_rel = this->_stateEstimatorData.leg_data[i].v;
